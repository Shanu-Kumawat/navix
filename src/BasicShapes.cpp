...
std::vector<ImVec2> ShockAbsorberEnd2D::generateProfile() const {
    std::vector<ImVec2> profile;
    float x = baseCenter.x;
    float y = baseCenter.y - shaftLength / 2.0f;
    // Step 3 (top, smallest)
    profile.push_back(ImVec2(x + step3Diameter / 2, y));
    y += step3Length;
    profile.push_back(ImVec2(x + step3Diameter / 2, y));
    // Step 2
    profile.push_back(ImVec2(x + step2Diameter / 2, y));
    y += step2Length;
    profile.push_back(ImVec2(x + step2Diameter / 2, y));
    // Step 1
    profile.push_back(ImVec2(x + step1Diameter / 2, y));
    y += step1Length;
    profile.push_back(ImVec2(x + step1Diameter / 2, y));
