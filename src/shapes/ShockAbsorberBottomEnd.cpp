...
void ShockAbsorberBottomEnd::updateGeometry() {
    ...
    baseCenter = ImVec2(parentSpring->centerX, parentSpring->centerY + fl / 2 + plateThickness / 2 + height / 2);
}
...
bool ShockAbsorberBottomEnd::isPointNear(const ImVec2& point, float threshold) const {
    ImVec2 min, max;
    getBounds(min, max);
...
void ShockAbsorberBottomEnd::getBounds(ImVec2& min, ImVec2& max) const {
    ...
    min = ImVec2(baseCenter.x - width / 2, shaftTopY);
    max = ImVec2(baseCenter.x + width / 2, baseCenter.y + plateThickness / 2);
}
...
void ShockAbsorberBottomEnd::draw(ImDrawList* drawList, const Canvas* canvas) const {
    ...
    float plateTopY = baseCenter.y - plateThickness / 2;
    float shaftTopY = parentSpring->centerY - fl / 2 + step1Length; // bottom of step1
    ImVec2 shaftTop(baseCenter.x - shaftWidth / 2, shaftTopY);
    ImVec2 shaftBot(baseCenter.x + shaftWidth / 2, plateTopY);
    ImVec2 shaftPts[4] = {tx(shaftTop), tx(ImVec2(shaftBot.x, shaftTop.y)), tx(shaftBot), tx(ImVec2(shaftTop.x, shaftBot.y))};
    drawList->AddPolyline(shaftPts, 4, c, ImDrawFlags_Closed, t);
    // Plate
    ImVec2 plateMin(baseCenter.x - width / 2, plateTopY);
    ImVec2 plateMax(baseCenter.x + width / 2, plateTopY + plateThickness);
    ImVec2 p1 = tx(plateMin), p2 = tx(ImVec2(plateMax.x, plateMin.y)), p3 = tx(plateMax), p4 = tx(ImVec2(plateMin.x, plateMax.y));
    ImVec2 platePts[4] = {p1, p2, p3, p4};
    drawList->AddPolyline(platePts, 4, c, ImDrawFlags_Closed, t);
    // U-bracket legs
    float legTop = plateTopY + plateThickness;
    float legBottom = legTop + height;
    // Left leg
    ImVec2 l1 = tx(ImVec2(plateMin.x, legTop)), l2 = tx(ImVec2(plateMin.x, legBottom));
    drawList->AddLine(l1, l2, c, t);
    // Right leg
    ImVec2 r1 = tx(ImVec2(plateMax.x, legTop)), r2 = tx(ImVec2(plateMax.x, legBottom));
    drawList->AddLine(r1, r2, c, t);
    // Bottom arc (semi-circle at the very bottom)
    ImVec2 arcCenter = tx(ImVec2(baseCenter.x, legBottom));
    ...
    // Hole
    ImVec2 holeCenter = tx(ImVec2(baseCenter.x, legTop + height / 2));
...
std::vector<ImVec2> ShockAbsorberBottomEnd::generateProfile() const {
    ...
    profile.push_back(ImVec2(leftX, plateTopY));
    profile.push_back(ImVec2(leftX, plateBottomY));
    ...
    profile.push_back(ImVec2(rightX, legBottom));
    profile.push_back(ImVec2(rightX, plateBottomY));
