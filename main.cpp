  ...
  if (ImGui::CollapsingHeader("Drawing Tools",
    ...
    if (ImGui::Button(ICON_SPLINE, ImVec2(columnWidth, buttonSize))) {
      SelectTool(Drawing::DrawingMode::Spline, canvas,
                 "Click to add control points, right-click to finish spline");
    }
    if (ImGui::IsItemHovered()) {
    ...
    ImGui::SameLine();
    bool isBezierActive =
        UIState::activeMode == Drawing::DrawingMode::BezierCurve;
    if (isBezierActive) {
      ImGui::PushStyleColor(ImGuiCol_Button, UIColors::BUTTON_ACTIVE);
    } else {
      ImGui::PushStyleColor(ImGuiCol_Button, UIColors::BUTTON);
    ...
    if (ImGui::Button(ICON_BEZIER, ImVec2(columnWidth, buttonSize))) {
      SelectTool(Drawing::DrawingMode::BezierCurve, canvas,
                 "Click to place 4 control points for Bezier curve");
    }
    if (ImGui::IsItemHovered()) {
...
void RenderCanvas(Drawing::Canvas &canvas) {
  ...
  ImGui::SetNextWindowSize(ImVec2(ImGui::GetIO().DisplaySize.x - 250,
